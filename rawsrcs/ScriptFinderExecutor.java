...
public class ScriptFinderExecutor implements CommandExecutor{
    private BlockAction instance;
    public ScriptFinderExecutor(BlockAction instance){
    ...
    public CommandResult execute(CommandSource source, CommandContext args) throws CommandException {
        ...
        if (!(source instanceof Player)) {
            if (!worldOptional.isPresent()) {
                source.sendMessage(Text.builder("You must specify which world you wish to perform this action on.!")
        ...
        if (handlerOptional.isPresent()) {
            ...
            for (Map.Entry<Vector3i, List<CommandScript>> entry : handler.getEntryset()) {
                ...
                if (entry.getKey().distanceSquared(now) <= range * range) {
        ...
        Text.Builder builder = Text.builder(blockposes.size() + " ")
                ...
                .append(Text.builder("scripts found in world ")
        ...
        for (int i = 0; i < blockposes.size(); ++i) {
            builder.append(Text.builder(blockposes.get(i).toString() + (i < blockposes.size() - 1 ? "\n":""))
                    .style(TextStyles.NONE)
